...
class LearningCard extends GeneralContainerElement {
  constructor(wordObj, setObj, handler) {
    super('div', '');        
  ...
  buildCard(settings) {
    ...
    repeatAudio.addStyles('none', 'no-pointer');
    
    faceSide.addChildren(imageContainer, wordContainer, BtnContainer);
    BtnContainer.addChildren(checkBtn, repeatAudio, audio);
  ...
  if (settings.showTranslateWord === true) {  
    ...
    backSide.addChildren(wordTranslationContainer);   
  }
  ...
  const wrapperAnswerBtns = new GeneralContainerElement('div', 'wrapperAnswerBtns');
    ...
    ButtonContainer.addChildren(wrapperAnswerBtns);
